...
input.on('line', (input) =>{
  ...
  var byte1 = Math.round(scale(input.substr(2, 2), 0, 100, 0, 127));
  var byte2 = Math.round(scale(input.substr(4, 2), 0, 100, 0, 127));
  ...
  // Send the MIDI data
  console.log('Parsed MIDI values: ' + byte1 + ' ' + byte2);

  output.sendMessage([176, 33, byte1]);
  output.sendMessage([176, 34, byte2]);
